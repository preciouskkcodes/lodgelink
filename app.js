...
// ════════════════════════════════════════════════════════════════
// AUTH — Phone number collection (no OTP provider needed for now)
// ════════════════════════════════════════════════════════════════
const AUTH_KEY = 'lodgelink_user';
...
// Called when user taps "Continue" on the phone login screen
window.sendOtp = function() {
  ...
  // Save user immediately — no OTP step since we have no SMS provider yet
  const user = { phone: trimmed, name: 'Guest ' + trimmed.slice(-4), loggedInAt: Date.now() };
  setAuthUser(user);
  showToast('Welcome to LodgeLink! 🎉', 'success');
  updateProfileUI();
...
function updateProfileUI() {
  ...
  if (!user) {
    ...
    if (phoneEl) phoneEl.textContent = 'Tap to sign in';
    return;
  }
  if (nameEl)  nameEl.textContent  = user.name  || 'LodgeLink Guest';
  if (phoneEl) phoneEl.textContent = user.phone || '';
}

window.handleLogoutApp = function() {
...
window.initGoogleMaps = function() {
  console.log('[LodgeLink] Google Maps SDK ready');
};

function buildMapPreviewHtml(location) {
  const q = encodeURIComponent((location || 'Asese, Ogun State, Nigeria'));
  const embedUrl = 'https://www.google.com/maps/embed/v1/place?key=YOUR_GOOGLE_MAPS_KEY&q=' + q + '&zoom=14';
  const mapsUrl  = 'https://maps.google.com/?q=' + q;
  return `
    <div class="map-preview">
      <iframe src="${embedUrl}" loading="lazy" allowfullscreen referrerpolicy="no-referrer-when-downgrade" title="Location map"></iframe>
      <button class="map-overlay-btn" onclick="window.open('${mapsUrl}','_blank')">Open in Maps &#8599;</button>
    </div>`;
}
